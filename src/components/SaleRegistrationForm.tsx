...
export default function SaleRegistrationForm() {
  ...
  const lookupSKU = useCallback(
    async (sku: string) => {
      ...
      try {
        ...
        if (!estoque) {
          setProduto(null); setSkuStatus('not_in_showcase'); setSkuMessage('Item não está no seu mostruário.'); return;
        }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setErrors({});
    if (!produto || skuStatus !== 'found' || !form.termo_aceito) {
      setErrors({ _global: 'Verifique o SKU e aceite os termos.' }); return;
    }
    setSubmitting(true);
    try {
      ...
      setErrors({ _global: err.message || 'Erro ao registrar venda.' });
